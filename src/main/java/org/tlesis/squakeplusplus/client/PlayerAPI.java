...
public class PlayerAPI {
    ...
    public static boolean isLiquidPresentInBB(Entity player, Box bb) {
        AtomicBoolean isInCollision = new AtomicBoolean(true);
        Stream<VoxelShape> collisions = player.world.getCollisions(player, bb, entity -> true);
        collisions.forEach(collision -> {
            if (isInCollision.get()) {
                if (!collision.isEmpty()) {
                    isInCollision.set(false);
                }
            }
        });
        return isInCollision.get() && !player.world.containsFluid(bb);
    }
}
